...
* Supported Actions:
    ...
    * Advanced Actions:
        * **Custom**: Gives the user the ability to inject the scenarios with custom Java code, for any reason
        ...
        at runtime (using the src/runtime package classes). Additional actions with custom code:
...
or write the Java code for testing (using the scr/midtier/Writer class), then execute the result file
-in both cases- (using the src/midtier/Executer class).
...
* Other minor changes:
    * 'test' package renamed to '**samples**'.
    * New samples added to (java samples/ reports/ xml files).
